...
- July 30, 2025. **REDENUMIRE COMPLETĂ PARTNERS/PARTENERI FINALIZATĂ** - Modificat complet terminologia în întreaga interfață aplicație de la "Beneficiari/Beneficiar" la "Parteneri/Partener". Actualizat toate etichetele, navigația sidebar, formularele, textele din interfață, mesajele de eroare și notificările. Eliminat câmpul `company_legal_representative` din baza de date și înlocuit cu `partners.name` pentru reprezentanții legali. Redenumit tabela bazei de date de la `beneficiaries` → `parteneri` → `partners` și rutele API de la `/api/beneficiaries` → `/api/parteneri` → `/api/partners`. Corectate toate referințele din frontend și backend, reparat importurile, și restaurat funcționalitatea completă a API-ului. **STATUS: COMPLET FUNCȚIONAL - TERMINOLOGIE UNIFORMĂ**
...
- July 29, 2025. **REMOVED CONTRACT VALUE TOTALS FROM DASHBOARD** - Simplified dashboard stats by removing total value calculations for signed and completed contracts. Reverted getContractStats function to basic counting only, removed currency formatting and financial totals from StatsCards component. Dashboard now displays clean contract counts without monetary values for streamlined overview.
- July 29, 2025. **DASHBOARD SIMPLIFIED WITH LATEST 5 CONTRACTS** - Redesigned dashboard to show only the latest 5 contracts without pagination for cleaner, focused view. Replaced full contract table with simplified card-based layout showing contract number, beneficiary name, creation date, status badge, and quick action buttons (view, edit, download). Added "Vezi toate contractele" button when 5+ contracts exist to navigate to full contracts page. Dashboard now provides streamlined overview of recent activity.
- July 29, 2025. **FIXED HANDLEBARS CONDITIONAL PROCESSING IN PDF** - Fixed critical bug where Handlebars conditional logic ({{#if isCompany}}, {{#if isIndividual}}) was appearing literally in PDF instead of being processed. Enhanced populateTemplate function with proper conditional processing that evaluates beneficiary type and shows appropriate content sections while removing unused conditional blocks. PDF generation now correctly displays different contract clauses for companies vs individuals without showing template syntax.
- July 29, 2025. **ENHANCED DIGITAL SIGNATURES WITH TIME DISPLAY** - Enhanced PDF generation with colored signature boxes including full date and time for signed contracts. Digital signatures enclosed in thin orange-red bordered boxes (0.5px) with blue signature icons positioned on right side. Signature display includes complete timestamp (DD.MM.YYYY HH:MM format) for precise signing audit trail. All signing information organized with smaller fonts and professional visual styling with company names, legal representatives, and unique tokens.
- July 29, 2025. **SIGNED CONTRACT PDF FILENAME FORMAT COMPLETED** - Successfully implemented PDF filename generation for signed contracts using format "CTR_{{order_number}}_{{signed_token}}.pdf" across all download locations. Updated handleDownload functions in both contracts.tsx and dashboard.tsx pages to detect signed status (statusCode='signed') and signedToken presence, applying appropriate filename format. All contract download buttons (from lists, modals, and pages) now use uniform filename logic: signed contracts get "CTR_77_OCFBvIKHEusqjthIUzqq_VVAwTlyCK8i.pdf" format while non-signed contracts continue using "contract-77.pdf" format.
- July 29, 2025. **CRON SCHEDULER FOR CONTRACT STATUS UPDATES** - Replaced simple interval-based updater with comprehensive CronScheduler system. Created cron-jobs.ts with updateExpiredContracts(), weekly cleanup, and monthly statistics functions. CronScheduler runs contract status updates every 6 hours, checking for signed contracts past their end date and updating them to "Finalizat". Added API endpoints (/api/cron/status, /api/cron/trigger/contracts) for monitoring and manual triggering. System provides detailed logging and error handling for all automated tasks.
- July 29, 2025. **SIGNED CONTRACT PAGE WITH AUTO STATUS UPDATE** - Created signed-contract.tsx page for viewing signed contracts via unique tokens, resolving 404 errors for signed contract links. Added automatic status update to "Finalizat" (completed) when contract reaches its end date, not on first access. Page displays comprehensive contract details, beneficiary information, provider data, signing audit trail, and download functionality.
