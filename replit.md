...
- July 29, 2025. **ENHANCED DIGITAL SIGNATURES WITH VISUAL ELEMENTS** - Enhanced PDF generation with visual signature boxes and handwritten-style signature icons for signed contracts. Digital signatures now enclosed in bordered boxes with 10px padding, include signature-like curved lines as visual indicators, and display all signing information (company/person name, signed by, date, token) in organized format with smaller fonts. Prestator and Beneficiar sections both feature identical visual styling with signature icons and professional bordered layout.
