...
- July 29, 2025. **DIGITAL SIGNATURES IN PDF GENERATION** - Enhanced PDF generation to include digital signatures for signed contracts. When contract status is "signed", PDF now displays actual signing information instead of blank signature lines. Prestator section shows company name, legal representative, signed date, and signing token. Beneficiar section shows company/person name, signed by name, signed date, and signed token. Unsigned contracts continue to show traditional signature lines. Digital signatures provide complete audit trail in PDF format.
