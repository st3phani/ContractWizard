...
- July 29, 2025. **SIGNED CONTRACT PDF FILENAME FORMAT** - Modified PDF filename generation for signed contracts to use format "CTR_{{order_number}}_{{signed_token}}.pdf". Updated server routes and all client-side download functions (contract-table, contract-modal, signed-contract page) to detect signed status and apply appropriate filename format. Non-signed contracts continue using standard "contract-{{order_number}}.pdf" format.
