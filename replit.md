...
- July 29, 2025. **SIGNED CONTRACT PAGE WITH AUTO STATUS UPDATE** - Created signed-contract.tsx page for viewing signed contracts via unique tokens, resolving 404 errors for signed contract links. Added automatic status update to "Finalizat" (completed) when contract reaches its end date, not on first access. Implemented ContractStatusUpdater service that runs hourly to check and update expired contracts. Page displays comprehensive contract details, beneficiary information, provider data, signing audit trail, and download functionality.
