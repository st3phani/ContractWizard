...
- July 30, 2025. **TERMINOLOGIE SCHIMBATĂ: BENEFICIARI → PARTENERI FINALIZATĂ** - Modificat complet terminologia în întreaga interfață aplicație de la "Beneficiari/Beneficiar" la "Parteneri/Partener". Actualizat toate etichetele, navigația sidebar, formularele, textele din interfață, mesajele de eroare și notificările. Eliminat câmpul `company_legal_representative` din baza de date și înlocuit cu `beneficiaries.name` pentru reprezentanții legali. Corectate toate referințele corupte din frontend și backend, reparat importurile, și restaurat funcționalitatea completă a API-ului. Schema de baze de date rămâne neschimbată pentru compatibilitate (tabela `beneficiaries` păstrată). **STATUS: COMPLET FUNCȚIONAL**
